...
def load_data(dataset: str, shuffle_columns=False, shuffle_samples=True, random_seed: int = seed):
    ...
    if dataset == 'MIMIC':
        data = pd.read_csv('../dataset/MIMIC/mimic3.csv')
        ...
        # feature_order = ['LOSgroupNum', 'NumNotes', 'NumOutput', 'NumDiagnosis', 'NumRx', 'NumProcEvents',
        # 'TotalNumInteract', 'NumChartEvents', 'NumInput', 'NumLabs', 'NumTransfers', 'NumCPTevents', 'age',
        # 'NumMicroLabs', 'NumCallouts', 'NumProcs']
        feature_order = ['LOSgroupNum', 'NumTransfers', 'NumDiagnosis', 'NumCallouts', 'NumChartEvents',
                         'TotalNumInteract', 'NumProcEvents', 'age', 'NumInput', 'NumNotes', 'NumRx', 'NumMicroLabs',
                         'NumProcs', 'NumLabs', 'NumCPTevents', 'NumOutput']
        ...
        data = pd.read_csv('../dataset/Breast/breast.csv')
        data.drop(["Unnamed: 32", "id"], inplace=True, axis=1)
        # Encode diagnosis column (M = 1, B = 0)
        data["diagnosis"] = [1 if i.strip() == "M" else 0 for i in data.diagnosis]

        # Define the order of the features
        # feature_order = ['diagnosis', 'texture_mean', 'concavity_mean', 'concave points_worst', 'texture_worst',
        # 'radius_se', 'area_se', 'symmetry_worst', 'perimeter_se', 'radius_worst', 'concave points_mean',
        # 'fractal_dimension_mean', 'smoothness_se', 'concavity_worst', 'symmetry_se', 'fractal_dimension_worst',
        # 'area_worst', 'compactness_mean', 'concave points_se', 'compactness_se', 'area_mean', 'texture_se',
        # 'fractal_dimension_se', 'smoothness_worst', 'perimeter_worst', 'compactness_worst', 'radius_mean',
        # 'perimeter_mean', 'smoothness_mean', 'symmetry_mean', 'concavity_se']
        feature_order = ['diagnosis', 'area_worst', 'area_mean', 'area_se', 'radius_mean', 'compactness_se',
                         'smoothness_mean', 'compactness_mean', 'concavity_mean', 'concave points_mean',
                         'symmetry_mean', 'fractal_dimension_mean', 'radius_se', 'texture_se', 'symmetry_worst',
                         'fractal_dimension_worst', 'concavity_se', 'concave points_se', 'symmetry_se',
                         'fractal_dimension_se', 'smoothness_worst', 'compactness_worst', 'concavity_worst',
                         'concave points_worst', 'smoothness_se', 'perimeter_se', 'texture_worst', 'texture_mean',
                         'radius_worst', 'perimeter_worst', 'perimeter_mean']
...
def load_dataset_config(dataset: str):
    with open('configs/config.json') as load_config:
        config = json.load(load_config)
    return config[dataset]


def load_model_config(model_name: str):
    filename = 'configs/' + model_name + '.yaml'
...
# For 3 parties
def feature_split(dataset='MIMIC', start_sample=0, start_feature=0, random_seed=seed):
    ...
    setting = load_dataset_config(dataset)
    ...
    # Passive party 1
    X_passive_1 = X[start_sample + p2_num_sample - shared_sample_2:
                    start_sample + p2_num_sample + p1_num_sample - shared_sample_2,
                  start_feature + p2_num_feature - shared_feature_2:
                  start_feature + p2_num_feature + p1_num_feature - shared_feature_2]
    y_passive_1 = y[start_sample + p2_num_sample - shared_sample_2:
                    start_sample + p2_num_sample + p1_num_sample - shared_sample_2, :]

    # Active party
    X_active = X[start_sample + p2_num_sample + p1_num_sample - shared_sample_2 - shared_sample_1:
                 start_sample + p2_num_sample + p1_num_sample + active_num_sample - shared_sample_2 - shared_sample_1,
               start_feature + p2_num_feature + p1_num_feature - shared_feature_2 - shared_feature_1:
               start_feature + p2_num_feature + p1_num_feature + active_num_feature - shared_feature_2 - shared_feature_1]
    y_active = y[start_sample + p2_num_sample + p1_num_sample - shared_sample_2 - shared_sample_1:
                 start_sample + p2_num_sample + p1_num_sample + active_num_sample - shared_sample_2 - shared_sample_1, :]

    # Second overlapping samples
    p2_shared = X_passive_2[p2_num_sample - shared_sample_2:p2_num_sample, :]
    p1_shared_2 = X_passive_1[:shared_sample_2, :]
    ...
    a_shared = X_active[:shared_sample_1, :]
